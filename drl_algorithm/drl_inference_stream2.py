...
from h_configs import SERVICE2_OUTPUT_PATH_LIST, SERVICE2_FOG_ENDPOINT, SERVICE2_CLOUD_ENDPOINT, \
...
def input_run_stream():
    ...
    slice_index = 0
    current_part = 0
    audio_path = f"input/streaming.mp3"
    subtitle_path = f"input/streaming.txt"
    ...
    subtitle_parts = [subtitle[i:i+3] for i in range(0, len(subtitle), 2)] # the average speaking speed rate is 150 wpm (2.5 words per second). we choose 2 words per second
    ...
    while idx < len(audio_raw_data):
        ...
        sr_audio_name = f"sr{service_id}.mp3"
        sr_subtitle_name = f"sr{service_id}.txt"
        out_sr_audio_name = f"{out_sr_folder}/{sr_audio_name}"
        out_sr_subtitle_name = f"{out_sr_folder}/{sr_subtitle_name}"
        ...
        slice_audio_name = f"sr{service_id}_{slice_index}.mp3"
        slice_subtitle_name = f"sr{service_id}_{slice_index}.txt"
        out_slice_audio_name = f"{out_slice_folder}/{slice_audio_name}"
        out_slice_subtitle_name = f"{out_slice_folder}/{slice_subtitle_name}"
        ...
        if abs(passed_time - 1.0) <= 0.1:
            ...
            audio_part.export(out_slice_audio_name, format="mp3")
            audio_parts[current_part] = []
            audio_part_len = len(audio_part) / 1000  # convert milliseconds to seconds
            audio_parts_len += audio_part_len

            # subtitle slice handling
            current_part_index = current_part if current_part < len(subtitle_parts) - 1 else len(subtitle_parts) - 1
            previous_subtitle_parts = subtitle_parts[current_part_index - 1] if current_part_index > 0 else subtitle_parts[current_part_index]
            current_subtitle_parts = subtitle_parts[current_part_index]
            next_subtitle_parts = subtitle_parts[current_part_index + 1] if current_part < len(subtitle_parts) - 1 else subtitle_parts[current_part_index]
            final_subtitle_parts = previous_subtitle_parts + current_subtitle_parts + next_subtitle_parts
            subtitle_part = ' '.join(final_subtitle_parts)
            subtitle_sr.append(subtitle_part)
            open(out_slice_subtitle_name, "w").write(subtitle_part)
            ...
            if (slice_index >= 6 or (slice_index < 6 and abs(audio_len - audio_parts_len) <= 0.5)):
                ...
                audio_sr.export(out_sr_audio_name, format="mp3")

                # subtitle sr handling
                subtitle_sr = ' '.join(subtitle_sr)
                subtitle_sr_len = len(subtitle_sr)
                open(out_sr_subtitle_name, "w").write(subtitle_sr)

                end_throughput_time = time.time() # end throughtput time before assinging envs
                slices_count = slice_index
                total_frames_smartgateway = len(audio_sr) / 1000 + subtitle_sr_len
                start_throughput_time = time.time()
...
def run_model_helper(service_id, timestap, smartgateway_commtimes, total_frames_smartgateway, start_throughput_time, end_throughput_time):
    ...
    service = Service2(
                ...
                input_path_list = [f"{input_stream_folder}/sr{service_id}/sr{service_id}.mp3", f"{input_stream_folder}/sr{service_id}/sr{service_id}.txt"], 
    ...
    file_name = f"{SERVICE2_OUTPUT_PATH_LIST[1]}_final.{SERVICE2_OUTPUT_PATH_LIST[2]}"
    src_path = SERVICE2_OUTPUT_PATH_LIST[0] + "/" + f"user{user.id}" + '/' + f"service{service.id}" + "/" + file_name
