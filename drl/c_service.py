...
from h_configs import SIMULATION, SERVICE_INPUT_SIZE
...
class Service:
    def __init__(self, id, user:User, sensitivity, slice_count, input_path_list, output_path_list, api_endpoint_list):
        ...
        self._make_slices()
        self.slices_size_map = self._set_slices_size()
    ...
    def get_input_size(self):
        if self.input_size is None:
            if SIMULATION:
                self.input_size = random.randint(SERVICE_INPUT_SIZE[0], SERVICE_INPUT_SIZE[1])
            else:
                input_size = 0
                for input_path in self.input_path_list:
                    input_size += os.path.getsize(input_path)
                self.input_size = input_size / 1_000_000 # convert bytes to MB
        return self.input_size
    
    def _set_slices_size(self):
        slices_size_map = {}
        if SIMULATION:
            slices_size_map = divide_unequal(self.get_input_size(), self.slice_count)
        else:
            for slice_index in range(self.slice_count):
                ...
                slice_size = slice_size / 1_000_000 # convert bytes to MB
    ...
    def do_action_with_metrics(self, slice_index, assigned_env):
        ...
        if SIMULATION:
            ...
            # total_bytes = end_net_io_counters.bytes_sent - start_net_io_counters.bytes_sent + \
            #             end_net_io_counters.bytes_recv - start_net_io_counters.bytes_recv
            # elapsed_time = end_time - start_time
            # network_bandwith = total_bytes / elapsed_time / 1_000_000 # convert bytes to MB
    ...
    def _get_slice_path(self, input_path, slice_index):
        ...
        if SIMULATION:
            ...
            slice_path += f'{input_file_name}_{input_file_ext}/'
